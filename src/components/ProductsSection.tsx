...
const products = [
  {
    ...
    name: 'Data-Mate',
    badge: 'Core Engine',
    badgeColor: 'bg-cyan-50 dark:bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 border-cyan-200 dark:border-cyan-500/20',
...
export default function ProductsSection() {
  return (
    <section id="products" className="py-24 bg-slate-50 dark:bg-slate-900/20 border-b border-slate-200 dark:border-white/5 relative">
      <div className="grid-lines absolute inset-0 opacity-100 dark:opacity-10 pointer-events-none" />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        >
          <p className="text-xs font-mono tracking-widest text-cyan-500 dark:text-cyan-400 uppercase mb-4">Current Products</p>
          <h2 className="text-4xl sm:text-5xl font-bold text-slate-900 dark:text-white mb-4">Live in production.</h2>
        ...
        <div className="grid lg:grid-cols-2 gap-8 mb-8">
          {products.map((product, i) => (
            <motion.div
              ...
              className={`flex flex-col p-8 rounded-2xl border border-slate-200 dark:border-white/5 bg-white dark:bg-slate-950/50 transition-colors ${product.borderHover}`}
            >
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-500 font-mono">{product.version}</p>
                </div>
              </div>
              ...
              <div className="mt-auto flex flex-wrap gap-2">
                {product.stack.map((s) => (
                  <span key={s} className="px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-white/5 text-xs text-slate-500 dark:text-slate-400">
                    {s}
                  </span>
                ))}
              </div>
        ...
        >
          {[
            ...
            <div key={item.label} className="flex items-start gap-3 p-5 rounded-xl border border-slate-200 dark:border-white/5 bg-white dark:bg-slate-950/30">
              <item.icon className="w-4 h-4 text-slate-400 dark:text-slate-500 shrink-0 mt-0.5" />
              <div>
                <p className="text-sm font-medium text-slate-800 dark:text-slate-200 mb-0.5">{item.label}</p>
        ...
        <motion.div
          ...
          className="p-6 rounded-2xl border border-slate-200 dark:border-slate-800/60 bg-gradient-to-r from-cyan-50 to-purple-50 dark:from-cyan-950/20 dark:to-purple-950/20 flex flex-col sm:flex-row items-center justify-between gap-4"
        >
          <p className="text-sm text-slate-600 dark:text-slate-400 text-center sm:text-left">
            <span className="text-slate-900 dark:text-white font-medium">Interested in licensing our products</span> for your organisation?
          </p>
          <a
            href="mailto:kittu.priyatham@gmail.com"
            className="shrink-0 inline-flex items-center gap-2 px-5 py-2.5 rounded-lg border border-cyan-300 dark:border-cyan-500/30 bg-cyan-50 dark:bg-cyan-500/10 text-sm text-cyan-700 dark:text-cyan-400 hover:bg-cyan-100 dark:hover:bg-cyan-500/20 transition-colors"
          >
